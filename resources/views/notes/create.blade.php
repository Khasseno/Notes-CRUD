...
@section('content')
    <div>
        <form action="{{ route('notes.store') }}" method="post">
            ...
            <div class="mb-3">
                <textarea class="form-control" name="body" id="body_input"
                          placeholder="Какие у Вас планы?">{{ old('body') }}</textarea>
                @error('body')
                <p class="text-danger"> {{ $message }} </p>
                @enderror
            </div>
            ...
            <div class="mb-1 form-group row">
                ...
                <div class="col-sm-10">
                    <input type="date" name="date" value="{{ old('date') ? old('date') : date('Y-m-d') }}"
